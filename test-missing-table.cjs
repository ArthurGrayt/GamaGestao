...
async function checkMissing() {
    const { count, error } = await supabase.from('non_existent_table_xyz').select('*', { count: 'exact', head: true });
    if (error) {
        console.log(`Confirmed: Querying missing table returns error: ${error.message}`);
    } else {
        console.log(`Unexpected: Querying missing table did NOT return error.`);
    }
}
